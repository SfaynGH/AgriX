...
export function DatePickerWithRange({
  ...
  const firstAvailableDate = soilMoistureData.reduce(
    (minDate, current) => {
      const currentDate = parseISO(current.date);
      return currentDate < minDate ? currentDate : minDate;
    },
    parseISO(soilMoistureData[0].date),
  );

  const lastAvailableDate = soilMoistureData.reduce(
    (maxDate, current) => {
      const currentDate = parseISO(current.date);
      return currentDate > maxDate ? currentDate : maxDate;
    },
    parseISO(soilMoistureData[soilMoistureData.length - 1].date),
  );
